...
def send_gmail_report(sender_email, app_password, recipient_email, report_data):
    ...
    try:
        ...
        for idx, item_title in enumerate(items_sample, 1):
            badge = "🔴 HOAX" if "[PENIPUAN]" in item_title or "[SALAH]" in item_title or "TurnBackHoax" in item_title else "🟢 FAKTA"
            items_html += f"""
        ...
        if not items_html:
            items_html = '<tr><td colspan="3" style="padding: 15px; text-align: center; color: #718096;">Tidak ada artikel duplikat baru yang ditambahkan hari ini.</td></tr>'
            ...
            <div class="container">
                ...
                <div class="content">
                    <div class="stat-card">
                        ...
                        <p style="margin: 0; font-size: 15px;">Sistem telah berhasil menjalankan <strong>Web Scraping Otomatis</strong> dan memperbarui model klasifikasi NLP & LSTM.</p>
                    </div>

                    <div class="stat-grid">
                        <div class="stat-cell">
                            <span style="font-size: 20px; font-weight: bold; color: #2b6cb0;">{inserted}</span><br>
                            <small style="color: #718096;">Artikel Baru</small>
                        </div>
                        <div class="stat-cell">
                            <span style="font-size: 20px; font-weight: bold; color: #e53e3e;">{tbh_cnt}</span><br>
                            <small style="color: #718096;">Hoax (TurnBackHoax)</small>
                        </div>
                        <div class="stat-cell">
                            <span style="font-size: 20px; font-weight: bold; color: #38a169;">{cnn_cnt + antara_cnt}</span><br>
                            <small style="color: #718096;">Fakta (CNN & Antara)</small>
                        </div>
                    </div>

                    <h4>📰 Sampel Artikel Terbaru yang Ditambahkan Hari Ini:</h4>
                    <table>
                        <thead>
                            <tr>
                                <th>No</th>
